        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    Unranked = discord.Embed(title=f"{account_name}님의 리그 오브 레전드 티어", description=f'[op.gg 바로가기](https://www.op.gg/summoners/kr/{account_name_url})', color=0xffdc16)
                    Unranked.add_field(name='UNRANKED', inline=True)
                    Unranked.set_thumbnail(url='https://cdn.discordapp.com/attachments/955355332983521300/964471258093846548/lol_icon.png')
                    await ctx.respond(embed=Unranked)       
        except:
            NoID = discord.Embed(title=f"{account_name}님을 찾을 수 없습니다.", description=f'[op.gg 바로가기](https://www.op.gg/)', color=0xffdc16)
            NoID.add_field(name='닉네임을 올바르게 입력하셨나요?', inline=True)
            NoID.set_thumbnail(url='https://cdn.discordapp.com/attachments/955355332983521300/964471258093846548/lol_icon.png')
            await ctx.respond(embed=NoID)      
def setup(bot):
